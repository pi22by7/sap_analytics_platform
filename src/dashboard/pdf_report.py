...
def generate_executive_report(data, output_path):
    ...
    # OTD Calculation
    grs = ekbe[ekbe["BEWTP"] == "E"].merge(ekpo[["EBELN", "EBELP", "EINDT"]], on=["EBELN", "EBELP"])
    ...
    spend_by_vendor = ekpo.merge(ekko[["EBELN", "LIFNR"]], on="EBELN") \
                          .groupby("LIFNR")["NETWR"].sum() \
                          .nlargest(5).reset_index()
    
    # Merge names
    spend_by_vendor = spend_by_vendor.merge(data["lfa1"](["LIFNR", "NAME1", "LAND1"]], on="LIFNR")
    ...
    recs = []
    if otd_rate < 95:
        recs.append("<b>Logistics Optimization:</b> OTD is below the 95% target. Initiate root cause analysis with bottom-quartile performers.")
    if compliance < 70:
        recs.append("<b>Spend Consolidation:</b> Contract compliance is below 70%. Enforce 'No PO, No Pay' policies to reduce maverick spend.")
    
    # Check for maverick spend magnitude
    maverick = ekko[ekko["BSART"] == "FO"]
    if not maverick.empty:
         recs.append(f"<b>Maverick Spend Control:</b> Identified {len(maverick)} off-contract orders. Transition top 20% of these commodities to framework agreements.")

    if not recs:
        recs.append("Operations are performing within target parameters. Maintain current monitoring cadence.")
